...
func main() {
	...
	// Endpoint to post a video for storage
	router.POST("/api.postvideo", postVideo)
...
func postVideo(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to send the video to the ML model"})
		return
	}
